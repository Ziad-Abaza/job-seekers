...
document.addEventListener("DOMContentLoaded", function () {
  ...
  xhr.open("GET", "chat/get_users.php", true);
  xhr.onload = () => {
      ...
      if (xhr.status === 200) {
        ...
        if (data) {
          ...
          data.forEach(function (user) {
            ...
            userDiv.innerHTML = `
                    <div style="cursor: pointer;" class="user_container gap-2 height-50px" onclick="sendUserData(${user.id});">
...
function sendUserData(user_id) {
  ...
  xhr.open("GET", `chat/test.php?user=${user_id}`, true);
  xhr.onload = function () {
    if (xhr.status == 200) {
...
function constructChatArea(userData) {
  ...
  chatArea.innerHTML = `
    ...
    grid-template-rows: auto 1fr auto; /* Header, chat messages, message input */
        ...
        <div class="p-2">
        <div class="chating_user d-flex gap-3 height-50px user_container">
            <div class="height-50px width-50px rounded-circle overflow-hidden">
