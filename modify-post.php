            ...
            <form method="post" action="modify-post.php" class="body-form w-100" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="title" name="title"
                        value="<?php echo $jobData['title']; ?>">
                </div>
                <div class="mb-3">
                    <label for="description" class="form-label">Job Description</label>
                    <textarea class="form-control" id="description" name="description"
                        rows="5"><?php echo $jobData['description']; ?></textarea>
                </div>
                <div class="container-xxl bg-white p-0">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="salary" name="salary"
                            value="<?php echo $jobData['salary']; ?>">
                    </div>
                    <div class="mb-3">
                        <label for="type" class="form-label">Job Type</label>
                        <select class="form-select" id="type" name="type">
                            <option value="full time" <?php if ($jobData['type'] == 'full time') echo 'selected'; ?>>
                                Full
                                Time</option>
                            <option value="part time" <?php if ($jobData['type'] == 'part time') echo 'selected'; ?>>
                                Part
                                Time</option>
                        </select>
                    </div>
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="category" name="category">
                            <option value="Web Development"
                                <?php if ($jobData['category'] == 'Web Development') echo 'selected'; ?>>Web Development
                            </option>
                            <option value="UI/UX Design"
                                <?php if ($jobData['category'] == 'UI/UX Design') echo 'selected'; ?>>UI/UX Design
                            </option>
                            <option value="App Development"
                                <?php if ($jobData['category'] == 'App Development') echo 'selected'; ?>>App Development
                            </option>
                            <option value="Software Development"
                                <?php if ($jobData['category'] == 'Software Development') echo 'selected'; ?>>Software
                                Development</option>
                            <option value="Database Administration"
                                <?php if ($jobData['category'] == 'Database Administration') echo 'selected'; ?>>
                                Database
                                Administration</option>
                            <option value="Network Engineering"
                                <?php if ($jobData['category'] == 'Network Engineering') echo 'selected'; ?>>Network
                                Engineering</option>
                            <option value="Embedded Systems"
                                <?php if ($jobData['category'] == 'Embedded Systems') echo 'selected'; ?>>Embedded
                                Systems
                            </option>
                            <option value="IoT" <?php if ($jobData['category'] == 'IoT') echo 'selected'; ?>>IoT
                            </option>
                        </select>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="d-flex gap-2 flex-wrap justify-content-evenly justify-content-md-start">
                        <button type="button" class="btn btn-secondary " onclick="addRequirement()">Add
                            Requirement</button>
